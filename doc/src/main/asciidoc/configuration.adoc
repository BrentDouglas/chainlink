...
[source,xml]
----
include::{chainlinkd}/cnf/chainlink.xml[]
----
...
injectable reference. Here is an example loading an Injector (that
...
[source,java]
----
include::{example}/TheInjector.java[]
----
...
order they are added. In the example we saw the injector defined in
xml moved after a second injector. In this case 'otherInjector' would
be called first and 'theInjector' second. The ordering may or may not
be important and you should check the javadoc to determine how they
are called. For example, specified Injector's are called one at a time
until on returns true, the ArtifactLoader's are called one at a time
until one return an artifact or throws an exception, while different
