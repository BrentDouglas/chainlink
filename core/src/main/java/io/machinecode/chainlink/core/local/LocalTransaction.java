...
public class LocalTransaction implements Transaction {
        ...
        try {
            ...
            switch (this.status) {
                case Status.STATUS_ACTIVE:
                    break;
                default:
                    throw new RollbackException(Messages.format("CHAINLINK-007000.transaction.cant.commit", status, _status(Status.STATUS_ACTIVE)));
            }
            if (this.timeout < end) {
                throw new RollbackException(Messages.format("CHAINLINK-007003.transaction.timed.out"));
            }
        ...
        try {
            switch (this.status) {
                ...
                case Status.STATUS_PREPARED:
                    throw new IllegalStateException(Messages.format("CHAINLINK-007001.transaction.cant.rollback", _status(status)));
            }
    ...
    public void setRollbackOnly() throws IllegalStateException, SystemException {
        if (this.status != Status.STATUS_ACTIVE) {
            throw new IllegalStateException(Messages.format("CHAINLINK-007002.transaction.cant.set.rollback.only", _status(status), _status(Status.STATUS_MARKED_ROLLBACK), _status(Status.STATUS_ACTIVE)));
        }
    ...
    public void registerSynchronization(final Synchronization sync) throws RollbackException, IllegalStateException, SystemException {
        ...
        switch (this.status) {
            ...
            case Status.STATUS_ROLLEDBACK:
                throw new RollbackException(Messages.format("CHAINLINK-007004.transaction.rolling.back", _status(status)));
            default:
                throw new IllegalStateException(Messages.format("CHAINLINK-007005.transaction.illegal.state", _status(status)));
        }
    }
    ...
    private void _before() {
        ...
        for (final Synchronization sync : this.syncs) {
            try {
                ...
                this.status = Status.STATUS_MARKED_ROLLBACK;
                log.tracef(e, Messages.get("CHAINLINK-007100.transaction.before.completion.exception"));
            }
        }
    }
    ...
    private void _after(final int status) {
        ...
        for (final Synchronization sync : this.syncs) {
            try {
                ...
                log.tracef(e, Messages.get("CHAINLINK-007101.transaction.after.completion.exception"));
            }
        }
        this.syncs.clear();
    }
