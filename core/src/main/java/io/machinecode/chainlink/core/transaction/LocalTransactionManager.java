...
public class LocalTransactionManager implements TransactionManager, UserTransaction {
    ...
    public void begin() throws NotSupportedException, SystemException {
        ...
        if (current != null) {
            throw new NotSupportedException(Messages.format("CHAINLINK-008002.transaction.manager.multiple.transactions.not.supported", Thread.currentThread(), current));
        }
    ...
    public void commit() throws RollbackException, HeuristicMixedException, HeuristicRollbackException, SecurityException, IllegalStateException, SystemException {
        try {
            ...
            if (transaction == null) {
                throw new IllegalStateException(Messages.format("CHAINLINK-008001.transaction.manager.no.transaction", Thread.currentThread()));
            }
            transaction.commit();
        } finally {
    ...
    public void resume(final Transaction transaction) throws InvalidTransactionException, IllegalStateException, SystemException {
        final Transaction current = this.transaction.get();
        if (current != null) {
            throw new IllegalStateException(Messages.format("CHAINLINK-008000.transaction.manager.existing.transaction", Thread.currentThread(), current));
        }
